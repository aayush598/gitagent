...
${runningContext}`;
	...
	const serverPassword = process.env.GITCLAW_PASSWORD || crypto.randomBytes(16).toString("hex");
	...
	function generateAuthToken(): string {
		return crypto.createHash("sha256")
			.update(`${serverUsername}:${serverPassword}:${sessionSalt}`)
			.digest("hex")
			.slice(0, 32);
	}
...
return false;
	...
	const httpServer: Server = createServer(async (req, res) => {
		...
		const allowedOrigin = origin && (origin.startsWith("http://localhost") || origin.startsWith("http://127.0.0.1") || origin.startsWith("http://[::1]"));
		...
		if (url.pathname === "/api/auth" && req.method === "POST") {
			if (!checkLoginRateLimit()) {
				return jsonReply(res, 429, { ok: false, error: "Too many login attempts. Try again later." });
			}
...
<p><a href="https://skills.sh" target="_blank">Open skills.sh in a new tab</a></p></div></body></html>`);
	...
	if (autoGenerated) {
		console.log(dim(`[voice] Auth: auto-generated password (prefix: "${serverPassword.slice(0, 4)}...") — set GITCLAW_PASSWORD to use a custom password`));
	}
	console.log(dim(`[voice] Auth: user "${serverUsername}"`));
	console.log(dim(`[voice] Open http://127.0.0.1:${port} in your browser`));
