...
|---|---|---|---|
| \texttt{logToolUse} (1/tool) | $\sim$300 bytes | 30 KB | 300 KB |
| \texttt{logToolResult} (1/tool) | $\sim$1.2 KB (result truncated to 1000 chars) | 120 KB | 1.2 MB |
| \texttt{logResponse} (1/turn) | $\sim$200 bytes | 20 KB | 200 KB |
| \textbf{Total per turn} | $\sim$1.7 KB | 170 KB | 1.7 MB |
| \textbf{Total for 10k turns} | 17 MB | — | — |
| \textbf{Total for 100k turns} | 170 MB | — | — |
| \textbf{Total for 1M turns} | 1.7 GB | — | — |

A voice server session handling 100k turns (not unrealistic for a 24-hour deployment) generates $\sim$170 MB of audit data. In a container with a 1 GB ephemeral disk, this can fill the disk in $\sim$6 hours.

\subsubsection*{The \texttt{catch} Block Swallows Disk-Full Errors}
...
\subsubsection*{No Existing Rotation Infrastructure}

The \texttt{audit.jsonl} file is never:
\begin{itemize}
    \item Rotated by size (e.g., split at 10 MB)
    \item Rotated by time (e.g., daily)
    \item Compressed (old rotations could be gzipped)
    \item Checked for available disk space before writing
    \item Capped at a maximum number of rotation files
\end{itemize}


\subsection*{Fix Implementation}
\subsubsection*{Option A: Size-Based Rotation with Retention Limit (Recommended)}
...
3. Retention limit (default 5 compressed files = $\sim$50 MB max total)
4. Non-fatal error handling that still logs the failure

\subsubsection*{Option B: Simple Truncation (Ring Buffer)}
...
\subsubsection*{Option C: Daily Rotation with Size Cap}
