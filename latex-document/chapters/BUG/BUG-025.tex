...
\subsubsection*{Problem 1: Global Scope Mutation}
...
\begin{itemize}
    \item \textbf{Internal libraries}: \texttt{ws} (WebSocket library), \texttt{node-cron} (scheduler), \texttt{js-yaml} (YAML parsing)
    \item \textbf{pi-ai} (the AI model library imported in \texttt{loader.ts})
    \item \textbf{Node.js built-in modules}: \texttt{stream}, \texttt{http}, \texttt{fs} may all emit warnings or errors via \texttt{console.error}/\texttt{console.warn}
    \item \textbf{Any user-loaded plugins} (via \texttt{src/plugins.ts}): Plugin code may log its own diagnostics
\end{itemize}

\subsubsection*{Problem 2: The \texttt{extractSource} Logic is Fragile}
...
\subsubsection*{Problem 3: No Way to Disable or Filter}

Once \texttt{installConsoleIntercept()} runs, there is no mechanism to:
\begin{itemize}
    \item Pause the intercept (e.g., during sensitive operations)
    \item Filter out messages from specific sources
    \item Uninstall the intercept and restore the original \texttt{console} methods
    \item Set a log level threshold for the intercept
\end{itemize}
...
\subsubsection*{Problem 4: Performance Overhead for Every Log Call}
...
\subsubsection*{Option A: Namespace-Specific Logger Instead of Global Patch (Recommended)}
...
\subsubsection*{Option B: Source-Based Filtering with Allowlist}
...
\subsubsection*{Option C: Restorable Patch with Uninstall}
