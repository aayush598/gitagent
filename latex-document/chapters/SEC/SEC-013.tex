...
\begin{itemize}
    ...
    \item \srcfile{test/plugin-deserialize.test.ts} --- Standalone test suite (14 tests) with no external dependencies.
\end{itemize}

\subsection*{Verification}
14 tests: 11 for \texttt{prevalidateManifest} (rejects \texttt{!!js/function}, \texttt{!!js/undefined}, \texttt{!!js/regexp}, \texttt{!!python/}, \texttt{!!ruby/}, \texttt{!!php/}; accepts safe manifest, anchors, base YAML types like \texttt{!!str}/\texttt{!!int}/\texttt{!!bool}, and empty strings) and 3 for manifest parsing (normal plugin, nested config, empty).
