...
\textbf{Commit} & 962eb56d007f4269ea0193faf23dfe2506e0a405 \\
\end{tabular}
...
\begin{itemize}
    ...
    \item \srcfile{src/__tests__/plugin-integrity.test.ts} --- New comprehensive test suite for integrity checking.
\end{itemize}

\subsection*{Verification}
Tests verify that \texttt{computeDirHash()} produces consistent output for identical directories, changes when file content changes, ignores the \texttt{.git} directory, and returns a 64-character hex string. Integration tests verify that \texttt{installPlugin()} throws an integrity check error when the expected hash does not match, succeeds when the hash matches, and installs without hash when \texttt{expectedHash} is not provided for backward compatibility.

\subsection*{Test File}
\srcfile{src/__tests__/plugin-integrity.test.ts}

Contains 7 test cases: \texttt{computeDirHash} consistency, hash changes on content modification, \texttt{.git} directory exclusion, hash format validation, \texttt{installPlugin} rejection on hash mismatch, successful installation on hash match, and backward-compatible installation without hash.
